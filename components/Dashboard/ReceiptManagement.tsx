...
export default function ReceiptManagement() {
  ...
  const fetchReceipts = async () => {
    try {
      ...
      const [receiptsRes, summaryRes] = await Promise.all([
        api.get(`/payment-receipts/?${params.toString()}`),
        api.get('/payment-receipts/summary/'),
      ]);
  ...
  const exportReceipts = async () => {
    try {
      ...
      const response = await api.get(`/payment-receipts/?${params.toString()}`);
                        ...
                        <td colSpan={7} className="px-6 py-4">
                          <div className="grid grid-cols-2 gap-4 text-sm">
                            <div>
                              ...
                              <p className="text-gray-900 font-medium">{receipt.receipt_number}</p>
                            </div>
                            <div>
                              <p className="text-gray-600">Amount</p>
                              <p className="text-gray-900 font-medium">D {parseFloat(receipt.amount).toLocaleString()}</p>
                            </div>
                            <div>
                              <p className="text-gray-600">Signatory</p>
                              <p className="text-gray-900 font-medium">{receipt.signatory_name}</p>
                            </div>
                            <div>
                            ...
                            <div>
                              ...
                              <p className="text-gray-900 font-medium">{receipt.generated_by_name}</p>
                            </div>
                            <div>
                              <p className="text-gray-600">Generated At</p>
                              <p className="text-gray-900 font-medium">{new Date(receipt.generated_at).toLocaleString()}</p>
                            </div>
                          </div>
                        </td>
                      </tr>
